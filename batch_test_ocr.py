...
cases = [c for c in cases if c["text"] in target_texts and c["suffix"] in target_suffixes]
...
for case in cases:
    ...
    try:
        ...
        match = expected.lower() in recognized.lower() or recognized.lower() in expected.lower()
        ...
        print(f"{status} [{suffix:12}] {case['degradation']:20} | 预期: {expected[:20]:20} | 实际: {recognized[:30]}")
    except subprocess.TimeoutExpired:
        print(f"✗ [{suffix:12}] {case['degradation']:20} | 超时")
        results.append({"file": Path(img_path).name, "expected": expected, "recognized": "(timeout)", "degradation": case["degradation"], "exact_match": False, "partial_match": False})
    except Exception as e:
        print(f"✗ [{suffix:12}] {case['degradation']:20} | 错误: {e}")
        results.append({"file": Path(img_path).name, "expected": expected, "recognized": f"(error: {e})", "degradation": case["degradation"], "exact_match": False, "partial_match": False})
...
for d, stats in sorted(by_degradation.items()):
    print(f"{d:25} | 总计: {stats['total']} | 精确: {stats['exact']} | 部分: {stats['partial']}")
