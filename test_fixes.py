...
def test_search_retry_decorator():
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "search", "__init__.py"), "r"
    ) as f:
        content = f.read()
...
def test_graceful_shutdown():
    ...
    assert (
        "shutdown" in src or "cleanup" in src.lower() or "_db_executor.shutdown" in src
    ), "lifespan 关闭阶段应清理资源"
...
def test_fetch_timeout_configured():
    ...
    assert "concurrent.futures.wait" in src, (
        "extract_multiple 应使用 wait 替代 as_completed"
    )
    print("  ✅ Fetch 超时配置")
...
def test_admin_api_auth():
    ...
    # 检查 admin API 路由存在
    admin_routes = [r.path for r in app.routes if hasattr(r, "path")]
    assert any("/admin/api/users" in r for r in admin_routes), (
        "admin API users 路由不存在"
    )
    assert any("/admin/api/stats" in r for r in admin_routes), (
        "admin API stats 路由不存在"
    )
...
def test_sec_m01_secret_key_required():
    ...
    for i, line in enumerate(lines):
        if "SECRET_KEY" in line and "os.environ.get" in line:
            ...
            assert "os.urandom" not in line, (
                f"SECRET_KEY 行不应使用 os.urandom 作为默认值: {line}"
            )
            break
    else:
    ...
    # 确认有 RuntimeError 检查
    content = "".join(lines)
    assert "raise RuntimeError" in content and "WWW_SEARCH_SECRET_KEY" in content, (
        "缺少 SECRET_KEY 时应报错"
    )
...
def test_cor_m08_no_double_usage():
    ...
    if chat_match:
        ...
        assert "record_usage" not in chat_src, (
            "非流式 chat 端点不应直接调用 record_usage"
        )
